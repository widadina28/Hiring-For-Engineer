...
class ConfirmViewModel: ViewModel() {
    ...
    fun callApiConfirm(){
        ...
        service.getHireByID(id).enqueue(object : Callback<AllConfirmResponse>{
            ...
            override fun onResponse(call: Call<AllConfirmResponse>, response: Response<AllConfirmResponse>) {
               val list = response.body()?.data?.map {
                   ConfirmModel(it.idHire.orEmpty(), it.nameCompany.orEmpty(),
                           it.projectName.orEmpty(), it.description.orEmpty(), it.image.orEmpty(), it.status.orEmpty())
